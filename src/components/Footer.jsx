export default function Footer({ content }) {
  return (
    <footer
      id="contacts"
      className="border-t border-white/6 bg-[radial-gradient(circle_at_top_right,rgba(196,155,43,0.12),transparent_26%),linear-gradient(180deg,var(--surface-dark)_0%,var(--surface-dark-strong)_100%)] py-20 text-white"
    >
      <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 md:grid-cols-2 xl:grid-cols-[1.1fr_0.7fr_0.9fr_0.8fr] lg:px-8">
        <div>
          ...
          <p className="mt-6 max-w-xs text-sm leading-7 text-white/55">{content.notice}</p>
        </div>

        <nav className="flex flex-col gap-4">
          <h3 className="text-sm font-extrabold uppercase tracking-[0.22em] text-[var(--accent-soft)]/85">
            Разделы
          </h3>
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-white/65">{content.address}</p>
        </div>
      </div>
